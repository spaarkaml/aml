...
Stage 0 and Stage 1 done (WP-1.1–1.7 + 1.1b; NAS pairing confirmed 2026-09-10). **Stage 2 complete** (WP-2.1–2.9: index, links, backlinks, tags, search, outline, templates + Daily, Boundings + Overview, palette + rebindable shortcuts; WP-2.10 Obsidian migration skipped by decision), 2026-09-11. Gate 2 manual QA (`docs/qa/stage-2.md`) is next, then Stage 3 (WP-2.10 Obsidian migration is skipped by decision). Gate 1 manual QA (`docs/qa/stage-1.md`) and the Windows install are Bryce's pending tasks. NAS setup steps are in `docs/06-NAS-SETUP.md`. `docs/CHANGELOG.md` is the per-WP log.

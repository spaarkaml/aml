...
**Built (WP-5.1):** exactly that, and the split is sharper than the assumption. `synopsis`, `label`, `status` (and `target_words`, `deadline`, `type` from earlier WPs) are keys in the document's own front matter — legible in the file, in the Properties panel and to anything else that reads markdown. The manifest holds **only** what the files cannot say for themselves: the Binder order, the compile exclusions and the Project's goal. There is no sidecar row to keep in step with a note, and a Project whose manifest is deleted is still a folder of markdown in alphabetical order.

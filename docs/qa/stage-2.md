...
| 15 | *(WP-2.6)* Open a long chapter; ⌘⇧I; click through the Outline; drag a section above another, then ⌘Z | Headings and nesting match the note; the highlight follows the caret as you scroll and type; the moved section takes its subheadings and text with it; one undo restores the order and the file on disk is clean markdown | ☐ |
| 16 | *(WP-2.7)* Write `_templates/daily.md` with a heading, `{{date:dddd D MMMM YYYY}}` and `[[{{yesterday}}]]`; press ⌘⇧D; press it again; page back a week in the Daily strip | The Daily is created at `journal/YYYY/` with the placeholders filled and the yesterday link resolving; pressing again reopens it without overwriting your writing; the strip dots the days you have written | ☐ |
| 17 | *(WP-2.7)* Add a second template (e.g. `Scene.md` with `{{title}}`), then ⌘K → "New Scene Note" | The command appears without a restart; the note is created beside the open note, named for the template with the rename ready, and its placeholders are filled | ☐ |
| 18 | *(WP-2.8)* **Design review.** Open the Overview beside the canvas | The arrangement (Folio heading, week bar, Bounding and Project tiles, recents) is what you want, or you note the changes — tiles vs a clustered constellation is still open | ☐ |
| 19 | *(WP-2.8)* Make Boundings for creative / academic / work / software; add notes from several folders with `+`; rename a note that is in one; search `bounding:<name>`; delete a Bounding | Counts and colours read at a glance; a renamed or moved note stays in its Bounding; the search returns exactly that Bounding's notes; deleting asks once and never touches the notes themselves | ☐ |
| 20 | *(WP-2.8)* On the PC, open the same Folio after a sync; add a note to the same Bounding from both machines | `.aml/boundings.yaml` merges without a conflict file; both notes are in the Bounding | ☐ |
| 21 | *(WP-2.9)* ⌘/ → rebind two shortcuts you actually want (try one that clashes on purpose); quit and relaunch; then ↺ one of them | The clash is refused and names the command holding the key; both rebindings still work after the restart; ↺ puts the default back | ☐ |
| 22 | *(WP-2.9)* ⌘K and skim the whole list | Everything you can do in AML is there — panels, Boundings, Dailies, templates, blocks, insert, index — and nothing is listed twice | ☐ |

...
| 4 | Click a note; then reopen the graph and **⌥-click** one | Click opens the note and closes the graph; ⌥-click re-centres the graph on that note without leaving | ☐ |
...
| 8a | Look at the whole-Folio graph as a picture | Each Bounding is a soft tinted region with its name over it; links curve; names do not pile on top of each other; hovering a note shows a small card and sets the rest back | ☐ |
| 8b | Pinch, two-finger scroll, then press **Show everything** (bottom right); hover a Bounding in the key | Pinch zooms about the pointer, two fingers pan, the window itself never zooms; Show everything glides back to the whole graph; the Bounding you hover comes forward | ☐ |
